...
<head>
    ...
    <style>
        ...
        .main-content {
            margin-top: 19px;
            margin-left: 220px; /* Added margin-left for main content */
        }
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="sidebarr">
                ...
                <form class="invite-form" method="POST" action="invite-users.php">
                    <input type="hidden" name="event_id" value="<?php echo $active_event_id; ?>">
                    <label for="invite_user_id">Select Users:</label>
                    <select name="invite_user_id[]" multiple>
                        <?php foreach ($users as $user) : ?>
                            <option value="<?php echo $user['id']; ?>"><?php echo htmlspecialchars($user['username']); ?></option>
                        <?php endforeach; ?>
                    </select>
                    <button type="submit" name="invite_button">Send Invite</button>
                </form>
            </div>
        </div>
        <div class="col-md-8">
            <div class="main-content">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-text">Created At: <?php echo htmlspecialchars($active_event['created_at']); ?></p>
                        <p class="card-text">Approved: <?php echo htmlspecialchars($active_event['approved']); ?></p>
                    </div>
                </div>

                <h5 class="mt-4">Invited Users</h5>
                <div class="invited-users-list">
                    <?php foreach ($invited_users as $invited_user) : ?>
                        <div class="invited-user">
                            <i class="fas fa-user"></i>
                            <span class="invited-user-name"><?php echo htmlspecialchars($invited_user['username']); ?></span>
                            <form method="POST" action="cancel-invite.php" style="margin-left:auto;">
                                <input type="hidden" name="invite_id" value="<?php echo $invited_user['invite_id']; ?>">
                                <button type="submit" class="cancel-invite-icon"><i class="fas fa-times"></i></button>
                            </form>
                        </div>
                    <?php endforeach; ?>
                </div>
...
<script>
    // Script to fade out the notification
    setTimeout(function() {
        var notification = document.querySelector('.invite-notification');
        if (notification) {
